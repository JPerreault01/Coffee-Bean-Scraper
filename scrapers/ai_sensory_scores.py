...
Usage (local, with venv active):
  ...
  python scrapers/ai_sensory_scores.py --only lavazza-super-crema --web-notes-file /tmp/notes.txt
...
def main():
    ...
    parser.add_argument("--web-notes-file", metavar="PATH",
                        help="Path to a text file containing third-party tasting notes "
                             "for the current bean (replaces the empty web_text placeholder). "
                             "Use with --only for per-bean orchestration.")
    ...
    for i, product in enumerate(products, 1):
        ...
        # Web search step: populated via --web-notes-file or left empty with --no-web.
        web_text = ""
        if not args.no_web and args.web_notes_file:
            notes_path = Path(args.web_notes_file)
            if notes_path.exists():
                web_text = notes_path.read_text(encoding="utf-8").strip()
                if web_text:
                    sources.append("web_notes_file")
